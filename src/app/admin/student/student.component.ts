...
export class StudentComponent implements OnInit {
  ...
  loadStudentClearances(student: Student) {
    // Use the global filter semester and academic year to get clearances for the selected period
    this.apiService.getClearancesByStudent(student.id, this.selectedSemester, this.selectedAcadYear).subscribe({
      next: (response) => {
        if (response.status.remarks === 'success' && response.payload) {
          ...
          // Recalculate status based on clearances for this semester
          this.updateStudentStatus(student);
        } else {
          // No clearances for this semester
          student.clearances = [];
          this.updateStudentStatus(student);
        }
      },
  ...
  // Calculate student status based on their clearances
  updateStudentStatus(student: Student) {
    if (student.clearances.length === 0) {
      student.status = 'Pending';
    } else {
      const allApproved = student.clearances.every(c => c.status === 'Approved');
      student.status = allApproved ? 'Approved' : 'Pending';
    }
  }
  ...
  toggleStudentSelection(student: Student): void {
    ...
    this.selectAll = this.filteredStudents.every(s => s.selected);
  }
